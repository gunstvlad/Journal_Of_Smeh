...
class NanoBananaAPI:
    def __init__(self):
        ...
        print(f"   Key: {settings.NANOBANANA_API_KEY[:15] if settings.NANOBANANA_API_KEY else 'EMPTY'}...")
            ...
            try:
                ...
                if hasattr(response, 'choices') and response.choices:
                    ...
                    if hasattr(message, 'images') and message.images:
                        ...
                        if image_url:
                            return {'image_url': image_url, 'revised_prompt': strategy['prompt']}
                    
                    # Проверяем content на наличие URL
                    if hasattr(message, 'content') and message.content:
                        urls = re.findall(r'https?://[^\s<>"\']+\.(?:png|jpg|jpeg|webp|gif)', message.content)
                        if urls:
                            print(f"🔗 Found URL in text content")
                            return {'image_url': urls[0], 'revised_prompt': strategy['prompt']}
    ...
    def _log_response(self, response):
        ...
        if response.choices:
            ...
            attrs = [a for a in dir(message) if not a.startswith('_') and not callable(getattr(message, a))]
